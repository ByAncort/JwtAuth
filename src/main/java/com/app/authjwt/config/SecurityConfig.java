...
import com.app.authjwt.config.jwt.JwtAuthenticationFilter;
    ...
    public UserDetailsService userDetailsService() {
        return username -> {
            ...
            user.getRoles().forEach(role -> {
                authorities.add(new SimpleGrantedAuthority( "ROLE_" + role.getName())); // ROLE_ADMIN

                //Convertir PERMISOS a autoridades (sin prefijo)
                role.getPermissions().forEach(permission ->
                        authorities.add(new SimpleGrantedAuthority(permission.getName())) // ADMIN_PERMISSIONS
                );
            });
